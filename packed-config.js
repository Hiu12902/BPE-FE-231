...
var endEventRequired = function() {
  ...
  function hasEndEvent(node) {
    ...
    return (
      flowElements.some(node => is$e(node, 'bpmn:EndEvent'))
    );
  }
  ...
  function check(node, reporter) {
    ...
    if (!hasEndEvent(node)) {
      const type = is$e(node, 'bpmn:SubProcess') ? 'Sub process' : 'Process';
...
var eventSubProcessTypedStartEvent = function() {
  ...
  function check(node, reporter) {
    ...
    if (!is$d(node, 'bpmn:SubProcess') || !node.triggeredByEvent) {
      return;
    }
    ...
    flowElements.forEach(function(flowElement) {
      ...
      if (!is$d(flowElement, 'bpmn:StartEvent')) {
        return false;
      }
...
var labelRequired = function() {
  ...
  function check(node, reporter) {
    ...
    if (is$c(node, 'bpmn:Gateway') && !isForking(node)) {
      return;
    }

    if (is$c(node, 'bpmn:BoundaryEvent')) {
      return;
    }

    // ignore sub-processes
    if (is$c(node, 'bpmn:SubProcess')) {
    ...
    if (is$c(node, 'bpmn:SequenceFlow') && !hasCondition$1(node)) {
      return;
    }
...
function isCompensationBoundary(node) {
  ...
  if (!is$9(node, 'bpmn:BoundaryEvent')) {
    return false;
  }
  ...
  return is$9(eventDefinitions[0], 'bpmn:CompensateEventDefinition');
}
...
var noDuplicateSequenceFlows = function() {
  ...
  function check(node, reporter) {
    ...
    if (!is$8(node, 'bpmn:SequenceFlow')) {
      return;
    }
...
utils.isEventSubProcess = isEventSubProcess$1;
utils.hasEventDefinition = hasEventDefinition$2;
...
const { isEventSubProcess, hasEventDefinition: hasEventDefinition$1 } = utils;

var noOutgoingFlow = function () {

  function check(node, reporter) {

    if (isAny(node, ['SequenceFlow', 'MessageFlow', 'ConditionalFlow', 'EndEvent']) || isEventSubProcess(node) || hasEventDefinition$1(node, 'bpmn:CompensateEventDefinition') || hasEventDefinition$1(node, 'bpmn:LinkEventDefinition')) {
      return;
    }

    if (!is$1(node, 'FlowNode')) {
      return;
    }
...
const { hasEventDefinition } = utils;
/**
 * A rule that checks the presence of a label.
 */
var noLinkCodeLinkEvent = function () {
...
cache['bpmnlint/no-outgoing-flow'] = noOutgoingFlow;
cache['bpmnlint/no-cycle-time-task'] = noCycleTimeTask;
cache['bpmnlint/no-link-code-link-event'] = noLinkCodeLinkEvent;
